...
public class AqiQualityWeekView extends View{
	...
	public AqiQualityWeekView(Context context) {
		super(context);
		mContext = context;
		init();
	}
	
	public AqiQualityWeekView(Context context, AttributeSet attrs) {
		super(context, attrs);
		mContext = context;
		init();
	}
	
	public AqiQualityWeekView(Context context, AttributeSet attrs, int defStyleAttr) {
...
//			maxTemp = 500;
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < tempList.size(); i++) {
			...
			textP.setTextSize(CommonUtil.dip2px(mContext, 12));
			float hourWidth = textP.measureText(CommonUtil.getDate(i));
			canvas.drawText(CommonUtil.getDate(i), dto.x-hourWidth/2, h-bottomMargin+CommonUtil.dip2px(mContext, 30), textP);
			
			textP.setColor(getResources().getColor(R.color.gray));
			textP.setTextSize(CommonUtil.dip2px(mContext, 12));
			float weekWidth = textP.measureText(CommonUtil.getWeek(mContext, i));
			canvas.drawText(CommonUtil.getWeek(mContext, i), dto.x-weekWidth/2, h-bottomMargin+CommonUtil.dip2px(mContext, 15), textP);
		}
